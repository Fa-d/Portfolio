...
const ManageStrings: React.FC = () => {
  const [content, setContent] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const dataType = 'strings'; // For API endpoint and messages
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSuccessMessage(`Site ${dataType.charAt(0).toUpperCase() + dataType.slice(1)} saved successfully!`);
    } catch (err) {
  ...
  return (
    <div>
      <h4>Manage Site {dataType.charAt(0).toUpperCase() + dataType.slice(1)}</h4>
      ...
      <div>
        <button onClick={handleSave} disabled={isLoading}>
          {isLoading ? 'Saving...' : `Save Site ${dataType.charAt(0).toUpperCase() + dataType.slice(1)}`}
        </button>
